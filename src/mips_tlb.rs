...
impl Tlb for MipsTlb {
    ...
    fn translate<const IS_64BIT: u8>(&mut self, virt_addr: u64, asid: u8, access_type: AccessType) -> TlbResult {
        ...
        // MRU linear scan (always used when tlbvmap is disabled; fallback for full
        // 64-bit VAs or ASID-aliased 32-bit VAs when tlbvmap is enabled).
    ...
    fn write(&mut self, index: usize, entry: TlbEntry) {
        if index < self.entries.len() {
            #[cfg(feature = "tlbvmap")] self.vmap_erase(index);
            self.entries[index] = entry;
            #[cfg(feature = "tlbvmap")] self.vmap_fill(index);
        }
    }
    ...
    fn power_on(&mut self) {
        ...
        #[cfg(feature = "tlbvmap")] self.vmap.fill(VMAP_MISS);
    }
